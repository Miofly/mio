<template>
	<view class="content">
		<scroll-view id="nav-bar" class="nav-bar"
			...
			<view v-for="(item,index) in tabBars" :key="item.id"
				  class="nav-item"
			...
			<swiper-item v-for="tabItem in tabBars" :key="tabItem.id">
				...
				<scroll-view class="panel-scroll-box" :scroll-y="enableScroll" @scrolltolower="loadMore">
					<view class="cu-list menu" :class="[false ? 'sm-border': '', false ? 'card-menu' : '']">
						<view v-if="tabCurrentIndex===0" class="cu-item" v-for="(item, index) in newsList" :key="index">
							<view class="text-center padding-top padding-bottom">
								{{item.title}}
								{{item.author}}
								{{item.time}}
								<view v-for="(image, index) in item.images" :key="index">
									<view>{{image}}</view>
								</view>
							</view>
						</view>
						<view v-if="tabCurrentIndex===1" class="cu-item" v-for="(item, index) in newsList" :key="index">
							<view class="text-center padding-top padding-bottom">
								{{item.title}}
							</view>
						</view>

    ...
    export default {
		...
		methods: {
			...
			async ssData () {
                this.appyys = []
                this.ui.showLoading()
                const data = await publicGet(`http://123.0t038.cn/jin-61/0509gkl/515love/api/getHomeInfo.php?type=1&page=${this.page}`)
                uni.hideLoading()
            ...
            mvDetail (url) {
				...
				console.log(this.$store.state.ssUrl)
                this.router.push({name: 'movieDetail'})
            },
		},
        async mounted() {
            this.ui.showLoading()
            const data = await publicGet('http://123.0t038.cn/jin-61/0509gkl/515love/api/getHomeInfo.php?type=1&page=1')
            uni.hideLoading()
            const $ = cheerio.load(data, {_useHtmlParser2: true})

    ...
    img.onload = function () {
        ...
        ewm.onload = function () {
            if (conf.txdata && conf.txdata.src) {
                ...
                ctx.drawImage(img, 0, 0)
                ctx.drawImage(img1, 100, 310, 560, 350)
                ctx.drawImage(ewm, conf.ewmdata.left, conf.ewmdata.top, conf.ewmdata.size, conf.ewmdata.size)
                if (conf.stringdata) {
                    for (var i in conf.stringdata) {
                        ctx.font = conf.stringdata[i].font
...
function setH(id) {
    ...
    // //套路列表将关怀编织成一缕风
    // 为妈妈轻舞飞扬/将祝福采集成一首诗/为妈妈轻轻吟唱/祝妈妈母亲节快乐，身体健康。
    // document.getElementById("list").style.display = "block";
}

// 实现将项目的图片转化成base64
function convertImgToBase64(url, urlTwo, callback) {
    var canvas = document.createElement('CANVAS')
    var ctx = canvas.getContext('2d')
    var img = new Image()
    img.crossOrigin = 'Anonymous'

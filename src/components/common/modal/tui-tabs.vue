<template>
	<view class="tui-tabs-view" :class="[isFixed?'tui-tabs-fixed':'tui-tabs-relative',unlined?'tui-unlined':'']"
		...
		<view v-for="(item,index) in tabs" :key="index" class="tui-tabs-item" :style="{width:itemWidth}"
			...
			<view class="tui-tabs-title"
				  ...
				  :style="{color:currentTab==index?selectedColor:color,fontSize:size+'rpx',lineHeight:size+'rpx',
	...
	sliderHeight+'rpx',borderRadius:sliderRadius,bottom:bottom,background:sliderBgColor,marginBottom:bottom=='50%'?
	('-'+sliderHeight/2+'rpx'):0}"></view>
	</view>
</template>
    ...
    export default {
        ...
        methods: {
            checkCor: function () {
                ...
                const padding = this.winWidth / 750 * this.padding
                const width = this.winWidth - padding * 2
                const left = (width / tabsNum - (this.winWidth / 750 * this.sliderWidth)) / 2 + padding
                let scrollLeft = left
...
<style>
	...
	.tui-tabs-relative::before {
		content: '';
		position: absolute;
		border-bottom: 1 upx solid #eaeef1;
